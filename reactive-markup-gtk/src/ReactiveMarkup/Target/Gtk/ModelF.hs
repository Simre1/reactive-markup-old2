...
initiateModel :: forall x. TransformFData x => x ID -> IO (x ModelF)
initiateModel = traverseF fD fN
  where
    ...
    fD (ID a) = do
      (d, t) <- SE.newDynamic a
      pure $ ModelF d t
    fN :: forall a. TransformFData a => ID (Nested a) -> IO (ModelF (Nested a))
    fN (ID a) = do
      m <- initiateModel a
      (d, t) <- SE.newDynamic m
      pure $ ModelF d $ SE.mapEventTrigger initiateModel t

modelToDynamic :: TransformFData x => x ModelF -> x (FunctorF (Dynamic Gtk))
modelToDynamic = mapF (\(ModelF a _) -> FunctorF (GtkDynamic a)) (\(ModelF a _) -> FunctorF $ GtkDynamic $ modelToDynamic <$> a)

modelToUpdate :: TransformFData x => x ModelF -> IO (x Update)
